...
def encodeProcess(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kw):
            ...
            resolved_fonts_dir = fonts_dir if fonts_dir else os.path.join(
                source_dir, 'fonts')
            ...
            if 'HEVC' in encodeTypes and subtitles_info:
                ...
                subsetFonts(subtitle_paths, resolved_fonts_dir,
                            resolved_font_out_dir, assfontsubset_path)

            file2del = []
            if sourceType == 'Web':
                subprocess.run([
                    ffmpeg_path, '-i', source, '-c:a', 'copy', '-vn',
                    source[:-len(extSource)] + '.m4a'
                ], shell=True)
                if not os.path.exists(source[:-len(extSource)] + '.m4a'):
                    raise FileNotFoundError(
                        f"Failed to create {source[:-len(extSource)]+'.m4a'}")
                file2del.append(source[:-len(extSource)] + '.m4a')
            elif sourceType == 'BD':
                subprocess.run(
                    [eac3to_path, source, source[:-len(extSource)] + '.flac'],
                    shell=True)
                if not os.path.exists(source[:-len(extSource)] + '.flac'):
                    raise FileNotFoundError(
                        f"Failed to create {source[:-len(extSource)]+'.flac'}")
                subprocess.run([
            ...
            for i in range(len(encodeTypes)):
                ...
                if encode_type == 'HEVC':
                    ...
                    if (out_name_templates) and (encode_type in out_name_templates):
                        custom_name = out_name_templates[encode_type].format(
                            base_in_name)
                    ...
                    mux_cmd.extend([
                        ...
                        mute_video, '--language', '0:jpn', '--default-track',
                        '0:yes', source[:-len(extSource)] + extAudio
                    ])

                    if subtitles_info:
                        for sub_cfg in subtitles_info:
                            sub_file_path = source[:-len(
                                extSource)] + f'.{sub_cfg.get("type")}.ass'
                            mux_cmd.extend([
                                "--language",
                                f"0:{sub_cfg.get('language', 'zho')}",
                                "--track-name",
                        ...
                        if font_out_dir and os.path.isdir(font_out_dir):
                            for filename in os.listdir(font_out_dir):
                                font_path = os.path.join(
                                    font_out_dir, filename)
                                if os.path.isfile(font_path):
                                    _, ext_font = os.path.splitext(filename)
                                    if ext_font.lower() in [
                                            '.ttf', '.ttc', '.otf', '.woff',
                                            '.woff2'
                                    ]:
                                        mux_cmd.extend([
                    ...
                    if not os.path.isfile(source[:-len(extSource)] + f'.{verName}.ass'):
                        raise FileNotFoundError(
                            'Your subtitle files are not ready yet!\nMissing ' +
                    ...
                    encodeParams[i] = [
                        ...
                        param_x264.format(
                            x264_path,
                            source[:-len(extSource)] + f'.mute.{verName}'),
                        [
                            mp4box_path, '-add',
                            source[:-len(extSource)] + f'.mute.{verName}.mp4',
                            '-add', source[:-len(extSource)] + '.m4a', '-new',
                            output_mp4
                        ],
                        output_mp4,
                        source[:-len(extSource)] + f'.{verName}.ass',
                    ...
                    if chapter:
                        encodeParams[i][2] = encodeParams[i][2][:-2] + [
                            '-chap', source[:-len(extSource)] + '.txt'
                        ] + encodeParams[i][2][-2:]
                    file2del.append(source[:-len(extSource)] +
                                    f'.mute.{verName}.mp4')
            encodes = []
            for i in encodeParams:
                encodes.append(
                    subprocess.Popen(i[1], stdin=subprocess.PIPE, shell=True))
            ...
            if rpc:
                for i in range(len(encodes)):
                    rpChecker(source,
                              encodeParams[i][3],
                              subtitle=encodeParams[i][4],
                              subrender=sub,
                              message=encodeTypes[i],
                              output=encodeParams[i][3] + '.rpc.txt')
            if create_torrent:
                for i in range(len(encodes)):
